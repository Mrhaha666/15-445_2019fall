...
HashJoinExecutor::HashJoinExecutor(ExecutorContext *exec_ctx, const HashJoinPlanNode *plan,
    ...
    : AbstractExecutor(exec_ctx), plan_(plan),
      jht_(HT("build_hash_table", exec_ctx_->GetBufferPoolManager(), jht_comp_, jht_num_buckets_, jht_hash_fn_)),
...
void HashJoinExecutor::Init() {
  ...
  while (left_->Next(&tuple)) {
    ...
    jht_.Insert(nullptr, hash_value, tuple);
  }
}
...
bool HashJoinExecutor::Next(Tuple *tuple) {
  ...
  while (right_->Next(&right_tuple)) {
    ...
    jht_.GetValue(nullptr, hash_value, &left_tuples);
